...
const Inventory = async () => {
  ...
  const styles = (index: any) => {
    return {
      bg: index === 0 ? 'bg-primary-0' : 'bg-white dark:bg-black-600',
      color: index === 0 ? 'text-white' : 'text-black-800 dark:text-black-200',
      titleColor: index === 0 ? 'text-white' : index >= 1 ? 'text-black-800' : 'text-danger',
      bgImage: index === 0 ? 'bg-primary-500' : 'bg-secondary-400 dark:bg-black-400',
    }
  }
